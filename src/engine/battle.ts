...
/** Slicing moves that benefit from Sharpness ability (1.5x power) */
const SLICING_MOVES = new Set([
  'Aerial Ace', 'Air Cutter', 'Air Slash', 'Behemoth Blade', 'Ceaseless Edge',
  'Cross Poison', 'Cut', 'Fury Cutter', 'Kowtow Cleave', 'Leaf Blade',
  'Night Slash', 'Psycho Cut', 'Razor Leaf', 'Razor Shell', 'Sacred Sword',
  'Slash', 'Secret Sword', 'Solar Blade', 'Stone Axe', 'X-Scissor',
]);
...
export class Battle {
  ...
  constructor(
    ...
    // Trigger switch-in abilities for both leads (faster lead first)
    this.leadSwitchInEvents = this.triggerLeadAbilities();
  }

  /** Events generated by lead switch-in abilities, consumed on first getTurnStartEvents() or processTurn() */
  leadSwitchInEvents: BattleEvent[] = [];

  private triggerLeadAbilities(): BattleEvent[] {
    const events: BattleEvent[] = [];
    // Determine order by base speed to avoid RNG consumption for ties at battle start
    const speed0 = this.getActivePokemon(0).stats.spe;
    const speed1 = this.getActivePokemon(1).stats.spe;
    const order = speed0 >= speed1 ? [0, 1] : [1, 0];
    for (const playerIndex of order) {
      const pokemon = this.getActivePokemon(playerIndex);
      this.handleSwitchInAbility(playerIndex, pokemon, events);
    }
    return events;
  }
  ...
  processTurn(action1: BattleAction, action2: BattleAction): BattleEvent[] {
    ...
    // Prepend lead switch-in ability events on the first turn
    if (this.leadSwitchInEvents.length > 0) {
      events.push(...this.leadSwitchInEvents);
      this.leadSwitchInEvents = [];
    }
  ...
  private getEffectiveSpeed(playerIndex: number): number {
    ...
    if (pokemon.ability === 'Slow Start' && pokemon.turnsOnField <= 5) speed = Math.floor(speed * 0.5);
  ...
  private executeMove(
    ...
    // Normalize: all moves become Normal-type (with 1.2x power boost in Gen 7+)
    if (attacker.ability === 'Normalize' && !isStruggle) {
    ...
    // Type immunity check BEFORE accuracy (immune moves don't "miss", they just don't affect)
    if (move.category !== 'Status' && !isStruggle) {
      const effectiveness = getTypeEffectiveness(
        move.type as PokemonType,
        this.getEffectiveTypes(defender)
      );
      if (effectiveness === 0) {
    ...
    const bypassesSub = move.flags?.sound || false;
    if (defender.substituteHp > 0 && move.category !== 'Status' && !bypassesSub) {
      const modifiers = this.getAbilityItemModifiers(attacker, defender, move);
      const defenderTypes = this.getEffectiveTypes(defender);
      const isCrit = move.willCrit || rollCritical(this.rng, move.critRatio ?? 1);
      const subResult = calculateDamage(
        attacker, defender, move, this.state.weather,
        this.rng, isCrit, modifiers, defenderTypes
      );
      const subDamage = subResult.finalDamage;
      ...
      if (defender.substituteHp <= 0) {
        ...
        this.addEvent(events, 'substitute_break', { pokemon: defender.species.name });
      } else {
        this.addEvent(events, 'substitute_hit', { pokemon: defender.species.name, damage: subDamage });
      }
      return;
    }
  ...
  private executeDamagingMove(
    ...
    const eff = getTypeEffectiveness(move.type as PokemonType, this.getEffectiveTypes(defender));
    if (eff > 1) {
    ...
    // Knock Off: remove defender's item (if not already consumed)
    if (move.name === 'Knock Off' && defender.isAlive && defender.item && !defender.itemConsumed) {
      const removedItem = defender.item;
      defender.item = null;
      defender.itemConsumed = true;
      this.addEvent(events, 'item_removed', {
  ...
  private executeStatusMove(
    ...
    if (move.name === 'Sleep Talk') {
      ...
      if (chosenMove.category === 'Status') {
        ...
        // Type immunity check for called move (e.g. Normal-type Body Slam vs Ghost)
        const calledEff = getTypeEffectiveness(
          chosenMove.type as PokemonType,
          this.getEffectiveTypes(defender)
        );
        if (calledEff === 0) {
          this.addEvent(events, 'immune', {
    ...
    if (move.name === 'Pain Split') {
      ...
      const attackerDiff = avgHp - attacker.currentHp;
      const defenderDiff = avgHp - defender.currentHp;
      attacker.currentHp = Math.min(avgHp, attacker.maxHp);
      defender.currentHp = Math.min(avgHp, defender.maxHp);
      if (attackerDiff > 0) {
        this.addEvent(events, 'heal', { pokemon: attacker.species.name, amount: attackerDiff });
      } else if (attackerDiff < 0) {
        this.addEvent(events, 'damage', {
          attacker: defender.species.name, defender: attacker.species.name,
      ...
      if (defenderDiff > 0) {
        this.addEvent(events, 'heal', { pokemon: defender.species.name, amount: defenderDiff });
      } else if (defenderDiff < 0) {
        this.addEvent(events, 'damage', {
          attacker: attacker.species.name, defender: defender.species.name,
    ...
    for (const effect of move.effects) {
      ...
      switch (effect.type) {
        ...
        case 'heal': {
          ...
          attacker.currentHp = clampHp(attacker.currentHp + healAmount, attacker.maxHp);
          this.addEvent(events, 'heal', { pokemon: attacker.species.name, amount: healAmount });
          // Roost: remove Flying type for the rest of the turn
          if (move.name === 'Roost' && (attacker.species.types as string[]).includes('Flying')) {
            attacker.volatileStatuses.add('roost');
          }
          break;
        }
  ...
  /** Get a Pokemon's effective types, accounting for Roost (removes Flying for the turn). */
  getEffectiveTypes(pokemon: BattlePokemon): PokemonType[] {
    const types = pokemon.species.types as PokemonType[];
    if (pokemon.volatileStatuses.has('roost')) {
      const filtered = types.filter(t => t !== 'Flying');
      // If pure Flying (e.g., Tornadus), becomes Normal
      return filtered.length === 0 ? ['Normal' as PokemonType] : filtered;
    }
    return types;
  }
  ...
  private checkAbilityImmunity(defender: BattlePokemon, move: MoveData, events: BattleEvent[]): boolean {
    ...
    if (defender.ability === 'Wonder Guard' && move.category !== 'Status') {
      const effectiveness = getTypeEffectiveness(moveType, this.getEffectiveTypes(defender));
      if (effectiveness <= 1) {
  ...
  private getAbilityItemModifiers(
    ...
    switch (attacker.ability) {
      ...
      case 'Guts':
        ...
        break;
      case 'Toxic Boost':
        if ((attacker.status === 'poison' || attacker.status === 'toxic') && move.category === 'Physical')
          mods.attackMod = (mods.attackMod || 1) * 1.5;
        break;
      case 'Flare Boost':
        if (attacker.status === 'burn' && move.category === 'Special')
          mods.attackMod = (mods.attackMod || 1) * 1.5;
        break;
      case 'Normalize':
        // 1.2x power boost for Normalize in Gen 7+
        mods.powerMod = (mods.powerMod || 1) * 1.2;
        break;
      case 'Iron Fist':
    ...
    if (attacker.ability === 'Slow Start' && attacker.turnsOnField <= 5) {
      if (move.category === 'Physical') {
        mods.attackMod = (mods.attackMod || 1) * 0.5;
      }
    }

    // Move-specific power modifiers
    if (move.name === 'Knock Off' && defender.item && !defender.itemConsumed) {
      mods.powerMod = (mods.powerMod || 1) * 1.5;
    }
    ...
    switch (attacker.item) {
      ...
      case 'Expert Belt':
        ...
        break;
      case 'Soul Dew':
        // Gen 7+: 1.2x power for Psychic and Dragon moves (any holder, not just Lati@s)
        if (move.type === 'Psychic' || move.type === 'Dragon') {
          mods.powerMod = (mods.powerMod || 1) * 1.2;
        }
        break;
    }
  ...
  private processEndOfTurn(events: BattleEvent[]): void {
    ...
    for (const playerIndex of order) {
      ...
      // Clear Roost's Flying-type removal at end of turn
      pokemon.volatileStatuses.delete('roost');
    }
  }
